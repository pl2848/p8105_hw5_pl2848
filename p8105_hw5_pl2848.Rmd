...
```{r}
BMD_unsolved_test = prop.test(1825, 2827, p = NULL,alternative = c("two.sided", "less", "greater"),conf.level = 0.95, correct = TRUE)
...
```{r}
homicide_list = list(homicide_df$unsolved, homicide_df$number_of_homicides) 

city_prop_test = function(x,y){
  
test_data = broom::tidy(prop.test(x, y, p = NULL,alternative = c("two.sided", "less", "greater"),conf.level = 0.95, correct = TRUE)) 

tibble(test_data = broom::tidy(test_data) %>% 
  select(estimate, conf.low, conf.high))

}

map2(homicide_list[1],homicide_list[2],city_prop_test)
